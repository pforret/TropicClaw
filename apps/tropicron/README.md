...
│   ├── cli-changed.sh    # Precheck helper: detect new CLI output
│   └── url-changed.sh    # Precheck helper: detect URL content changes
├── skills/
│   ├── tropicron/SKILL.md       # Claude Code skill for /tropicron
│   └── tropicron-memory/SKILL.md # Documentation skill for per-job memory
├── examples/
│   ├── health-check.md   # Simple liveness check
│   └── scheduler-ping.md # Shell-only heartbeat (no LLM)
